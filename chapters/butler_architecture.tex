...
At the time of writing (April 2019), these metrics are evaluated as in Table \ref{tab:open_source_framework_viability}.
...
For example, \mintinline{sh}{salt -G 'roles:worker' state.apply airflow.patch-db} applies the \mintinline{text}{airflow.patch-db} state to all VMs that have the \mintinline{text}{worker} role.
...
\begin{description}
\item [consul] - A framework used for Service Discovery, which will be described in detail in Section \ref{sec:design_consul}
...
\begin{figure}[h]
    \includegraphics[scale=0.55]{butler_chronograf_alert_barchart}
    \centering
...
\begingroup
\captionof{listing} {Storing GPG encrypted data in a salt pillar variable. \label{lst:gpg_encrypted_pillar}}
\begin{minted}
[
breaklines=true,
linenos,
...
When the pillar variable is targeted to the VM that runs the self-healing agent and needs to be able to execute terraform commands, the salt-master uses its copy of the private key to decrypt the file contents and deliver them to the appropriate VM as a file (see Listing \ref{lst:gpg_decrypted_pillar}) that is ready to be consumed by terraform.

\begingroup
\captionof{listing} {Retrieving contents of a GPG encrypted pillar variable and storing them in a file. \label{lst:gpg_decrypted_pillar}}
\begin{minted}
[
breaklines=true,
linenos,
