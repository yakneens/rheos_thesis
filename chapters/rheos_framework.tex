...
Armed with the update rule for integrating read evidence we are ready to define a data stream operation of Local State Aggregator type is responsible for processing a stream of aligned reads (or read pairs), updating its local state with the genetic variation evidence contained in the reads, and emitting a stream of updated genomic loci (see Fig. \ref{fig:main_body_locus}) that can be used by downstream services for variant calling.
...
\begin{table}[!ht]
    ...
    {\begin{tabular}{l|p{12cm}}
    \toprule
    Inputs & \hangindent=1em$M_{aln} = \{m_i: m_i = (header, r)\} \text{ where } r = (s\_id, ref\_id, r\_id, b, q, f_p, rname, pos, mapq, cigar, flags)$. \\
    \cline{2-2}
...
\begin{description}
    ...
    \item[gl\_2] - Likelihood of homozygous reference genotyp at this locus, based on the data seen so far.
